...
n_min = min(n_min, abelli_n.min())
n_max = max(n_max, abelli_n.max())
...
n_min = min(n_min, vande_n.min())
n_max = max(n_max, vande_n.max())
...
f = open('/gdrive/projects/SL2S_hierarch/evol_nfw_alpha_sigma_only.dat', 'r')
chain = pickle.load(f)
f.close()
...
vdisp_mu_samp = chain['vdisp_mu'][burnin:][samp]
vdisp_mdep_samp = chain['vdisp_mdep'][burnin:][samp]
vdisp_zdep_samp = chain['vdisp_zdep'][burnin:][samp]
...
vvdeppars = np.zeros((nz, 2))
mvdeppars = np.zeros((nz, 2))
...
for i in range(nz):
    vvdeppars[i] = dm.fit_sigma_only(np.log10(vpop.veldisp[:, i]), np.log10(vpop.aimf[:, i]))[0]
    mvdeppars[i] = dm.fit_sigma_only(np.log10(mpop.veldisp[:, i]), np.log10(mpop.aimf[:, i]))[0]

for i in range(nson):
    #mstar_from_vdisp_samp = (2.4 - vdisp_mu_samp - vdisp_zdep_samp*(z[i] - 0.3))/vdisp_mdep_samp + 11.5
    #maimf_samp = aimf_samp + zdep_samp*(z[i] - 0.3) + mdep_samp*(mstar_from_vdisp_samp - 11.5)
...
pylab.plot(z, mvdeppars[:, 0], label='$M_*$ model')
pylab.plot(z, vvdeppars[:, 0], label='$\sigma$ model')
...
print 'kv_min:', splev(n_min, kvir_spline), n_min
print 'kv_max:', splev(n_max, kvir_spline), n_max
...
pylab.errorbar(spin_z, spin_aimf, yerr=spin_aimf_err, xerr=0.025, fmt='+', marker='s', color='r', label='Spiniello et al. 2014')
...
pylab.scatter(gar_z, gar_upper_limits, color=(1., 0.2, 1.), marker='v', label='Gargiulo et al. (2014)')
pylab.scatter(abelli_z[abelli_z<2.], abelli_upper_limits, color=(0., 2./5., 1./5.), \
               marker='v', label='Belli et al. (2014)')

for i in range(nlabels):
    this_sample = vande_samples == i
    if len(vande_z[this_sample]) > 0:
	pylab.scatter(vande_z[this_sample], vande_upper_limits[this_sample], color=vande_colors[i], \
               marker='v', label=vande_labels[i])

pylab.xlabel('$z$', fontsize=fsize)
pylab.ylabel('$\log{\\alpha_{\mathrm{IMF}}}$', fontsize=fsize)
pylab.ylim(-0.5, 0.7)
pylab.xlim(0., 2.)
...
ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.35), numpoints = 1, ncol=3, fontsize=10, scatterpoints=1)
#pylab.legend(loc='upper center', numpoints = 1, ncol=3, fontsize=10, scatterpoints=1)
